...
func (*OrderServer) NewOrderFunc(ctx context.Context, req *proto.NewOrder) (*emptypb.Empty, error) {
	empty := &emptypb.Empty{}
	...
	if err := barrer.CallWithDB(db, func(tx *sql.Tx) error {
		sql := fmt.Sprintf("insert into  `order` values(null, ?, ?,?)")
		_, err := tx.Exec(sql,  1,1, 0)
		if err != nil {
...
func (*OrderServer) UpdateOrderStatus(ctx context.Context, req *proto.OrderStatus) (*emptypb.Empty, error) {
	...
	if err := barrer.CallWithDB(db, func(tx *sql.Tx) error {
		sql := fmt.Sprintf("update order set is_deleted=? where id=1")
		_, err := tx.Exec(sql, 1)
		if err != nil {
			return err
		}
